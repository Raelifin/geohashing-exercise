...
function reportOpenPriceFromYahoo(callback) {
	if (typeof callback === 'undefined') {
		return rp(yahooDowPriceURL).then(function(body) { return getOpenPriceFromYahooResults(body); });
	} else {		
...
function geoHash(lat, lon, callback) {
	if (typeof callback === 'undefined') {
		return reportOpenPriceFromYahoo().then(function(price) {
			return buildGeoHash(lat,lon,new Date(),price);
		});
	} else {

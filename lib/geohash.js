...
function buildGeoHash(lat, lon, dateObject, openPrice) {
	...
	var newLat = parseInt(lat)+'.'+generatedCoords[0]; //ParseInt is used to cut off fractional component
	var newLon = parseInt(lon)+'.'+generatedCoords[1];
	return {lat: parseFloat(newLat), lon:parseFloat(newLon)};
}
...
function reportOpenPriceFromYahoo() {
	return rp(yahooDowPriceURL).then(function(body) {
		return getOpenPriceFromYahooResults(body);
	});
}

function geoHash(lat, lon, callback) {
	var coords = buildGeoHash(lat,lon,new Date(),reportOpenPriceFromYahoo());
	result = {lat: parseFloat(coords.lat.toFixed(6)), lon: parseFloat(coords.lon.toFixed(6))};
	callback(result);
}
